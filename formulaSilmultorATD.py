...
#     cv2.waitKey(1)
    ...
    upper_color_bar_bw=cv2.resize(upper_color_bar_bw, (84, 84), interpolation = cv2.INTER_NEAREST)
...
def compute_steering_speed_gyro_abs(a):
  ...
  #  plt.show()
    ...
    steering = (right_steering - left_steering +1.0)/2 #(right_steering - left_steering + 1.0)/2
...
class SilumationDetails:
    ...
    def PlotSteering(self):
        steering=np.array(self.gyroDiagram, dtype=None, copy=True, order='K', subok=False, ndmin=0)
        plt.plot(np.arange(self.numOfSteps),steering, 'ro')
        plt.axis([0,self.numOfSteps, min(steering)-5,max(steering)+5])
        plt.show()    

if __name__ == "__main__":
    mySilmulator=SilumationDetails(100,400)
    for i in range(0,mySilmulator.GetNumOfSteps()):
        mySilmulator.DoNextAction([0,10,0])

    mySilmulator.PlotSpeed()
